...
function toFormData(obj, formData, options) {
    ...
    if (value && !path && typeof value === "object") {
      if (utils_default.endsWith(key, "{}")) {
        ...
        arr.forEach(function each(el, index) {
          !utils_default.isUndefined(el) && formData.append(indexes === true ? renderKey([key], index, dots) : indexes === null ? key : key + "[]", convertValue(el));
        });
        return false;
      }
    }
  ...
  function build(value, path) {
    ...
    utils_default.forEach(value, function each(el, key) {
      const result = !utils_default.isUndefined(el) && visitor.call(formData, el, utils_default.isString(key) ? key.trim() : key, path, exposedHelpers);
      if (result === true) {
